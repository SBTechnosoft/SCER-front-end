...
var CurrentPath = CurrentPathName.substring(0, CurrentPathName.lastIndexOf("/") + 1)+"app/views/QuickMenu/DocumentScan/";
...
function _printUploadedFiles(info) {
    ...
    if (info.indexOf('DWTUploadFileName') != -1) {
        ...
        if (savedIntoToDB) {
            ...
            url += CurrentPath + 'app/views/QuickMenu/DocumentScan/action/UploadedImages/' + encodeURI(fileName);
        }
...
function upload_preparation(_name) {
    ...
    switch (document.getElementById("ddlLanguages").options.selectedIndex) {
        ...
        default: strActionPage += "php.php"; break;
    }
    DWObject.IfSSL = Dynamsoft.Lib.detect.ssl;
